...
typedef struct
{
    ...
    uint32_t    mix_audio_context_type;
                                                     ...
                                                     uint32_t context_type, void *mix_audio_stop_func)
{
    ...
    if (audio_prompt_env.isMixPromptOn)
    {
        ...
        audio_prompt_env.mix_audio_context_type     = context_type;
...
void audio_prompt_stop_playing(void)
{
    ...
    context_type = audio_prompt_env.mix_audio_context_type;
    ...
    if(audio_prompt_env.mix_audio_stop_func)
    {
        ...
        func_handler(audio_prompt_env.pendingStopOp,audio_prompt_env.mix_audio_con_id,audio_prompt_env.mix_audio_context_type);

        audio_prompt_env.pendingStopOp = 0;
        audio_prompt_env.mix_audio_con_id = 0;
        audio_prompt_env.mix_audio_context_type = 0;
...
void app_tws_cmd_prompt_play_req_handler(uint8_t *ptrParam, uint32_t paramLen)
{
    ...
    {
        if (app_ble_is_connection_on_by_index(ptPlayReq->dev_id))
        {
            return;
        }
    }
