...
POSSIBLY_UNUSED static APP_CROSS_CHIP_OTA_ROLE_E tota_cross_chip_ota_role = CROSS_CHIP_OTA_ROLE_IS_DONGLE;
#else
POSSIBLY_UNUSED static APP_CROSS_CHIP_OTA_ROLE_E tota_cross_chip_ota_role = CROSS_CHIP_OTA_ROLE_IS_MASTER;
#endif
...
void ota_control_update_MTU(uint16_t mtu)
{
    ...
    mtu = (mtu / 16) * 16 - 4;
#endif
    ota_control_env.dataPacketSize = mtu;
...
static void _handle_received_data(uint8_t *otaBuf, bool isViaBle,uint16_t dataLenth)
{
    ...
    switch (packet->packetType)
    {
...
#else
                #define SEND_TO_DONGLE_DATA_NUM_MAX   (sizeof(tota_cross_chip_ota_role)+sizeof(ota_control_env.accumulateCurrentReciveNumber))
                ota_control_env.accumulateCurrentReciveNumber = ota_control_env.alreadyReceivedDataSizeOfImage;
                app_tota_store_written_sum_image_size_to_flash(ota_control_env.accumulateCurrentReciveNumber);
                uint8_t send_to_dongle_code[SEND_TO_DONGLE_DATA_NUM_MAX] = {0};
                uint8_t *send_to_dongle_code_p = send_to_dongle_code;
                *send_to_dongle_code_p = tota_cross_chip_ota_role;
                memcpy(send_to_dongle_code_p+1,&ota_control_env.accumulateCurrentReciveNumber,
                    sizeof(ota_control_env.accumulateCurrentReciveNumber));
                BLE_send_custom_command(OP_DONGLE_TOTA_RECEIVE_DATA,
                                        send_to_dongle_code,
                                        sizeof(tota_cross_chip_ota_role)+sizeof(ota_control_env.accumulateCurrentReciveNumber),
            ...
            if (BES_OTA_START_MAGIC_CODE == packet->payload.cmdResumeBreakpoint.magicCode)
            {
                ...
                if (!memcmp(otaUpgradeLog.randomCode, packet->payload.cmdResumeBreakpoint.randomCode,
                    ...
                    sizeof(otaUpgradeLog.randomCode)) && memcmp(otaUpgradeLog.randomCode, zeroCode, sizeof(zeroCode)))
                    {
                        ...
                        memcpy(randomCode, otaUpgradeLog.randomCode, sizeof(otaUpgradeLog.randomCode));
        ...
        case OTA_COMMAND_CONFIG_OTA:
        {
            ...
            Bes_enter_ota_state();
            ASSERT((dataLenth - 1)==sizeof(OTA_FLOW_CONFIGURATION_T), "plaese check config len. %d != %d",\
