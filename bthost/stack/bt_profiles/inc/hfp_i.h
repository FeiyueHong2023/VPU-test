...
void hf_register_sco_codec_info_sync_handler(void (*cb)(const void* remote, uint8_t codec));
...
bool hfp_hf_send_command_do(struct hshf_control *hfp, hfp_response_func_t resp_cb, const char *data, unsigned int len, bool is_cust_cmd ,uint8 param);
#define hfp_hf_send_command(h,rc,d,dl) hfp_hf_send_command_do(h,rc,d,dl,false,0xFF)
#define hfp_hf_send_command_private(h,rc,d,dl,param) hfp_hf_send_command_do(h,rc,d,dl,false,param)
