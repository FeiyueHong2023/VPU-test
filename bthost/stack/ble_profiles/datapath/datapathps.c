    ...
    uint8_t sec_lvl, uint8_t user_prio, const void* params, const void* p_cb)
{
    ...
    datapathps_env_t* datapathps_env =
            (datapathps_env_t* ) ke_malloc(sizeof(datapathps_env_t), KE_MEM_PROFILE);
    ASSERT(datapathps_env, "[%s] datapathps_env is NULL", __func__);
...
#ifdef RC06_HEADSET_CONNECT_WITH_TB_DONGLE
        datapathps_env->shdl = 22;
#else
    ...
    TRACE(1, "DATAPATHPS uid %d shdl %d", datapathps_env->user_lid, datapathps_env->shdl);
...
uint16_t datapathps_gatt_get_start_handle(void)
{
    datapathps_env_t *datapathps_env = PRF_ENV_GET(DATAPATHPS, datapathps);
    return datapathps_env->shdl;
}
