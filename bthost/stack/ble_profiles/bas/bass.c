...
/**
 ****************************************************************************************
 * @brief Handles reception of the @ref BASS_BATT_RESTORE_BOND_DATA_REQ message.
 * @param[in] msgid Id of the message received (probably unused).
 * @param[in] p_param Pointer to the parameters of the message.
 * @param[in] dest_id ID of the receiving task instance (probably unused).
 * @param[in] src_id ID of the sending task instance.
 ...
 ****************************************************************************************
 */
__STATIC int bass_restore_bond_data(ke_msg_id_t const msgid,
                            struct batt_restore_bond_data_req const *p_param,
                            ke_task_id_t const dest_id,
                            ke_task_id_t const src_id)
{
    bass_env_t *p_bass_env = PRF_ENV_GET(BASS, bass);
    ...
    if (p_bass_env != NULL)
    {
        // Check if indicated connection is valid and established
        if (gapc_is_estab(p_param->conidx))
        {
            // Keep provided client configuration
...
#ifdef BLE_STACK_PORTING_CHANGES
    {BASS_BATT_RESTORE_BOND_DATA_REQ, (ke_msg_func_t) bass_restore_bond_data},
#endif
